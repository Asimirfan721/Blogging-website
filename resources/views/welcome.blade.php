...
<body class="bg-yellow-100 dark:bg-yellow-900 text-[#1b1b18] flex flex-col min-h-screen">
                ...
                <nav class="flex items-center gap-4">
                    @auth
                        <a href="{{ route('posts.create') }}"
                            class="inline-flex items-center px-4 py-2 bg-[#f53003] hover:bg-[#d42802] text-white rounded-md font-semibold text-xs uppercase tracking-widest transition">
                            Create Post
                        </a>
                        <a href="{{ route('dashboard') }}"
                            class="px-4 py-2 border text-[#1b1b18] dark:text-[#EDEDEC] border-[#19140035] hover:border-[#1915014a] dark:border-[#3E3E3A] dark:hover:border-[#62605b] rounded-md text-sm transition">
                            Dashboard
                        </a>
                    @else
                        <a href="{{ route('login') }}"
                            class="px-4 py-2 text-[#1b1b18] dark:text-[#EDEDEC] hover:text-[#f53003] dark:hover:text-[#FF4433] text-sm transition">
                            Log in
                        </a>
                        @if (Route::has('register'))
                            <a href="{{ route('register') }}"
                                class="px-4 py-2 bg-[#f53003] hover:bg-[#d42802] text-white rounded-md text-sm font-medium transition">
                                Sign Up
                            </a>
                        @endif
                    @endauth
    ...
    <section class="bg-gradient-to-br from-[#4F46E5] to-[#7C3AED] py-20 px-6">
        <div class="max-w-4xl mx-auto text-center">
            <h1 class="text-xl text-blue-100 mb-8 font-bold italic font-serif">
                Create a blog<br> worth sharing
            </h1>
            <p class="text-xl text-blue-100 mb-8 font-bold italic font-serif">
                Get a full suite of intuitive design features and powerful marketing tools to create a unique blog that
                leaves a lasting impression.
            </p>

            @auth
                <a href="{{ route('posts.create') }}"
                    class="bg-white text-[#4F46E5] px-8 py-4 rounded-full text-lg font-semibold hover:bg-gray-100 transition">
                    Start Blogging
                </a>
            @else
                <a href="{{ route('register') }}"
    ...
    <main class="flex-1 w-full max-w-6xl mx-auto py-12 px-6">
        ...
        @if ($posts->isEmpty())
            <div class="text-center py-16">
                ...
                @auth
                    <a href="{{ route('posts.create') }}"
                        class="bg-[#f53003] hover:bg-[#d42802] text-white px-6 py-3 rounded-md font-medium transition">
        ...
        @else
            <div class="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
                @foreach ($posts as $post)
                    <article
                        class="bg-white dark:bg-[#161615] rounded-xl shadow-sm hover:shadow-md transition-shadow p-6 border border-[#e3e3e0] dark:border-[#3E3E3A]">
                        ...
                        <a href="{{ route('posts.show', $post->slug) }}"
                            class="inline-flex items-center text-[#f53003] dark:text-[#FF4433] hover:underline font-medium text-sm">
                            Read more
